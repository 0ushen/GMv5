...
int dll_display(DLL *dLL, int mode, int order, void(*pdisplay)(void*))
{
    ...
    for(i = 0 ; i < dLL->length ; i++)
    {
        /* Cette condition saute les données déjà lues si la fonction a été appellée
        ...
        /* Cette condition saute les données non lues si la fonction a été appellée
        ...
        // Affiche les données et change le statut des données en "lues".
